            ...
            <StaggerContainer className="space-y-3">
              {socialLinks.map((social) => (
                <StaggerItem key={social.label}>
                  <Button asChild variant="outline" className="w-full justify-start group bg-transparent hover:bg-primary hover:text-primary-foreground hover:border-primary">
                    <Link href={social.href} target="_blank" rel="noopener noreferrer">
                      <social.icon className="mr-3 h-4 w-4 group-hover:scale-110 transition-transform" />
                      <div className="text-left flex-1">
                        <div className="font-medium">{social.label}</div>
                        <div className="text-xs text-muted-foreground group-hover:text-primary-foreground/80">{social.description}</div>
                      </div>
